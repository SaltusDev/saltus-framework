...
Forty-one review findings across Phases 15, 16, and 17 — the complete backlog. Nothing here has been tagged; the observability surface described under [2.1.1] is what a 2.1.1 site actually runs.
...
### Security
	...
	- Two relationships that each name the other as `reciprocal` now store one row. Neither side is synthesized in that configuration, so both stayed forward sides reading `from_post_id`: one relationship was stored as two half-relationships, a cross-side read returned nothing, and a cross-side detach reported success while leaving the row in place. The lower-sorting endpoint owns the row and the other reads it in reverse, tied to the endpoint sort rather than model load order so a new plugin or a changed hook priority cannot reverse existing rows. The shared row's `capabilities` resolve to the owner's rule when it declared one and the far side's otherwise — a declared rule is never dropped, since the side left without one would be a read path around it — and both sides carry the survivor. A self-referential pair reached the same break by deriving two different keys, because equal endpoints leave the name pair unordered; those mismatched *derived* keys now reconcile, while an explicit `key` on either side is left alone so no stored row moves.
	- A privacy erasure no longer misses cascade dependents behind a relationship read rule. Cascade discovery ran through the read-filtered API, so a cascade the erasing operator could not read was skipped in silence and the request reported completion with the dependent content still held. Discovery now reads unfiltered and the rule is reported rather than bypassed: `erase_others_personal_data` is authority over every relationship, so a read rule blocking one is a contradictory configuration, and overriding it quietly would hide that mistake as surely as skipping quietly hid the incomplete erasure. The erasure retains and names the relationship through core's eraser messages. Only cascade-declaring relationships are considered, so one an erasure would never follow raises nothing.
