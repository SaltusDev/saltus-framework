...
### Changed
	...
	- Known top-level model config keys are the seven a model config is actually read for, each carrying a comment naming the reader that proves it. The list derived from every registered service id, so all nineteen counted as valid when twelve are only meaningful under `features` and one is not a model key at all — the unknown-key warning and the generated reference both accepted keys nothing reads. Deriving was wrong in one direction and a hand-written short list proved wrong in the other, so both failures are recorded where the next change will see them.
	...
	- The generated config key reference matches the restricted top-level key set, dropping the twelve features-only ids and the one non-model id it advertised.
