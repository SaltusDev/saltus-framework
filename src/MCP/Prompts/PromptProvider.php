...
class PromptProvider {
	...
	*/
	public function get( string $name, array $arguments = [] ): ?array {
		switch ( $name ) {
			case 'create_content':
				$postType = $arguments['post_type'] ?? 'posts';
				$topic    = $arguments['topic'] ?? '';
				$tone     = $arguments['tone'] ?? 'professional';

				return [
					'description' => 'Create content in the ' . $postType . ' post type',
					'messages'    => [
						[
							'role'    => 'user',
							'content' => [
								'type' => 'text',
								'text' => 'Create a new ' . $postType . ' post' . ( $topic ? ' about ' . $topic : '' ) . ' with a ' . $tone . ' tone. Use the create_post tool to publish it.',
							],
						],
					],
				];
			...
			case 'analyze_content':
				$postId = $arguments['post_id'] ?? null;

				return [
					'description' => 'Analyze post #' . ( $postId ?? '?' ) . ' for content quality',
					'messages'    => [
						[
							'role'    => 'user',
							'content' => [
								'type' => 'text',
								'text' => 'Fetch post #' . ( $postId ?? '?' ) . ' using the get_post tool, then analyze its title, content length, excerpt quality, and status. Suggest specific improvements for SEO and readability.',
							],
						],
					],
				];
