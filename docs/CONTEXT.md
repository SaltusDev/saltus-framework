...
- **Surface:** `WebMcp` feature service, `WebMcpController` routes (`/webmcp/manifest`, `/webmcp/execute`), `bridge.js` namespace-probing loader, filters `saltus/framework/webmcp/tools`, `saltus/framework/webmcp/manifest`, `saltus/framework/webmcp/public_fields`, and the discovery doc `docs/discovery/webmcp.md`.
